...
@Component({
  selector: 'app-row-delivering',
  templateUrl: './row-delivering.component.html',
  styleUrls: ['./row-delivering.component.css']
})
export class RowDeliveringComponent implements OnInit {
  @Input()
  commande!: Commande;
  isLivrable!: any;
  ...
  constructor(private router: Router, private transformationService: TransformationService, private dataService: DataService, private commandeService: CommandeService, private deliveringService: DeliveringService) { }
  ...
  isTakable(commande: Commande) {
    return commande.statut == StatutCommande.traite ? true : false
  }


  onChecked(inputLivrable: HTMLInputElement, commande: Commande) {
    // console.log(inputLivrable.checked, commande.id);
    if (inputLivrable.checked) {
      this.deliveringService.ajouterCommandeALivrer(commande)
    } else {
      this.deliveringService.enleverFromCommandeALivrer(commande)
    }
    console.log(this.deliveringService.commandeALivrer);
  }
